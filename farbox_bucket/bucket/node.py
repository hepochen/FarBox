...
remote_ips_config_filepath = '/mt/web/configs/backup_from_ips.txt'
def get_remote_nodes_to_sync_from():
    # 从本地配置中，引入要同步的节点 list，每一行一条记录; 简单有效的是，每行一个 ip
    # 如果 node 本身没有 :<port> 的声明， 默认会认为是 :7788 端口
    nodes = []
    if not os.path.isfile(remote_ips_config_filepath):
        return nodes
    with open(remote_ips_config_filepath, 'rb') as f:
        raw_content = f.read()
